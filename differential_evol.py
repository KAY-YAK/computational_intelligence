...
def diff_evolve_rand_1_exp(count, dim, low , high, budget, func, factor,crossover_rate):
        count_gen = 1
        # Generate population
        ...
        while(budget_no < budget):
                ...
                for index in range(length):
                        ...
                        #crossover
                        offspring = crossover_exponential(pop[index],mutation,crossover_rate)
...
time_ini = time.time()
print(diff_evolve_rand_1_exp(100,10,-5.12,5.12,5000,de_jong,.75,.9))
elapsed_time = (time.time() - time_ini) * 1000
print(elapsed_time)

time_ini = time.time()
print(diff_evolve_rand_1_exp(100,10,-5.12,5.12,5000,rastrigin,.75,.9))
elapsed_time = (time.time() - time_ini) * 1000
print(elapsed_time)

time_ini = time.time()
print(diff_evolve_rand_1_exp(100,10,-500,500,5000,schwefel,.75,.9))
elapsed_time = (time.time() - time_ini) * 1000
print(elapsed_time)

time_ini = time.time()
print(diff_evolve_rand_1_exp(100,10,0,m.pi,5000,michalewicz,.75,.9))
elapsed_time = (time.time() - time_ini) * 1000
print(elapsed_time)
# DO SAME FOR 50 D
